...
const registerUser = asyncHandler(async (req, res) => {
    ...
    console.log("Data of req.files file ", req.files);
...
const loginUser = asyncHandler(async (req, res) => {
    ...
    // find the user 
    // password check
    // access and refresh token grnerate 
    // send cookies 
    // resonse 

    const { email, username, password } = req.body
    // if you want to lofin with username or email so use this logic
    if (!(username || email)) {
        throw new ApiError(400, "username or email is required")
    }
    // if you want to login with both so you have to check both like 
    // (!username && !email)
